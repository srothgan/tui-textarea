...
fn wrapped_cursor_down_crosses_logical_line_boundary() {
    ...
    // Down: visual 1 → visual 2 (crosses into second logical line, col 5 clamped to line len 2)
    textarea.move_cursor(CursorMove::Down);
...
fn wrapped_cursor_up_crosses_logical_line_boundary() {
    ...
    // Up: visual 2 → visual 1 (col 0 clamped to start_col=5)
    textarea.move_cursor(CursorMove::Up);
    assert_eq!(textarea.cursor(), (0, 5));

    // Up: visual 1 → visual 0 (col 5 clamped to max_col=4 for non-last segment)
    textarea.move_cursor(CursorMove::Up);
    assert_eq!(textarea.cursor(), (0, 4));
...
fn wrapped_cursor_column_preserved_across_visual_lines() {
    ...
    // Down: logical col 2 is below visual line 1's start_col=5, so clamped to 5
    textarea.move_cursor(CursorMove::Down);
    assert_eq!(textarea.cursor(), (0, 5));
}
...
fn wrapped_cursor_column_clamped_to_shorter_visual_line() {
    ...
    // Down: col 4 is below visual 1's start_col=5, so clamped up to 5
    textarea.move_cursor(CursorMove::Down);
    assert_eq!(textarea.cursor(), (0, 5));
}
...
fn wrapped_cursor_down_up_with_word_wrap_mode() {
    ...
    // Up: col 6 clamped to visual line 0's max (end_col-1 for non-last segment)
    textarea.move_cursor(CursorMove::Up);
    assert_eq!(textarea.cursor(), (0, 5));
}
...
fn wrapped_cursor_three_visual_lines_from_one_logical() {
    ...
    // Up: col 10 clamped to visual 1's max_col=9 (non-last segment)
    textarea.move_cursor(CursorMove::Up);
    assert_eq!(textarea.cursor(), (0, 9));

    // Up: col 9 clamped to visual 0's max_col=4 (non-last segment)
    textarea.move_cursor(CursorMove::Up);
    assert_eq!(textarea.cursor(), (0, 4));
}
